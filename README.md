...
`/work/` and the case-study pages are generated by `make_projects.py` from
the `PROJECTS` table at its head — the output is committed and hand-editable;
re-run only when that table changes. It also rewrites the six home-page tile
links, so slugs cannot drift apart between the two pages.

Eight projects, all real. This is deliberately an interim set — further
projects get appended to `PROJECTS` as they exist, never backfilled with
placeholders. An odd count is safe: the layout pairs what it can and closes
the page with a full-width tile.
