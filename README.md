...
`/work/` and the fifteen case studies are generated by `make_projects.py` from
...
Eight project titles are real. The remaining seven keep this repo's own
placeholder vocabulary (Film Three, Series Four, Identity Five) rather than
inventing client names — replace them in `PROJECTS` and re-run.
